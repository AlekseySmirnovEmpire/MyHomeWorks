...
void doGetRequest(const std::vector<cpr::Pair>& pair) {

	std::string arguments = "?";

	for (int i = 0; i < pair.size(); ++i) {

		arguments += pair[i].key + '=' + pair[i].value + '&';
	}
...
void doPostRequest(const std::vector<cpr::Pair>& pair) {
	...
	do {
		...
		if (userInput.length() > 0) {
			...
			if (name == "post" || name == "get") {
				...
				reqs.push_back(cpr::Pair(name, value));
				clear();
			}
		}
		else {
